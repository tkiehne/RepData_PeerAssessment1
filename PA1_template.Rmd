...
The mean number of steps taken by this subject per day is `r meanspd`.  The median number of steps taken per day by this subject is `r medspd`. 

The total number of steps recorded by this subject over the observation period is `r sum(spd, na.rm=T)`.
...
The 5-minute interval, on average across all the days in the dataset, that contains the maximum number of steps for this subject is `r maxmeaninterval` which corresponds to `r str_pad(maxmeaninterval, 4, pad = "0")` hours in 24-hour time.
...
The total number of NA values for this subject during the observation period is `r numNA`, which accounts for `r (numNA/nrow(rawdata))*100`% of the data set.
...
The mean number of steps taken by this subject in the imputed data set per day is `r simmeanspd`.  The difference in means between the raw and imputed data is `r (abs(simmeanspd - meanspd)/mean(c(simmeanspd, meanspd)))*100`%.

The median number of steps taken by this subject in the imputed data set per day is `r simmedspd`.  The difference in median between the raw and imputed data is `r (abs(simmedspd - medspd)/mean(c(simmedspd, medspd)))*100`%.
...
The total number of steps taken by this subject in the imputed data set is `r sum(simspd)`, which is significantly more than the recorded number of steps in the original data set.  This difference accounts for the increased magnitude of the y-axis values in the imputed data set's steps per day histogram but has no effect on the distribution.
